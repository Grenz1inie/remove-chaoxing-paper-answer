...
提升超星学习通刷题效率的强大工具！支持**答案自由控制**（单个/全局）、**一键问豆包AI**（智能跨域提问+会话复用）、**富文本笔记**、**错题记录**（星级显示）、**完整样式自定义**、**灵活试题导出**（可配置DOC/DOCX格式），让你在作业练习中更专注、更高效。
...
### 🐛 Bug 修复
- **笔记按钮状态修复**：修复点击“显示笔记”后按钮颜色未立即变为紫色的问题
  - 点击后立即更新为紫色背景 + “隐藏笔记”文字
  - 鼠标悬停时不再改变颜色，保持当前状态颜色
- **编辑按钮颜色修复**：修正编辑/预览按钮颜色逻辑
  - 预览模式：橙色背景 + “编辑”文字
  - 编辑模式：绿色背景 + “预览”文字
  - 鼠标悬停时不再改变颜色
- **错题按钮样式统一**：将错题按钮悬停效果统一到StyleGenerator管理
  - 使用`addSimpleHoverEffect`方法管理悬停效果
  - 提高代码可维护性和一致性

### 🔧 技术优化
- **新增`addNoColorChangeHoverEffect`方法**：为按钮添加悬停动画但不改变背景色
- **代码重构**：统一按钮样式管理逻辑，提升代码质量
...
- **豆包自动发送修复**：修复未指定会话ID时豆包页面不自动发送的问题
  - 添加1.5s初始等待确保页面完全加载（解决新会话URL重定向问题）
  - 添加300ms输入后等待确保内容稳定后再点击发送
- **间距优化**：统一错题按钮和星星的间距为2px，界面更紧凑
- **星星弹性布局**：改进星星容器布局逻辑
  - 星星容器使用flexbox布局，出现时自动撑开空间
  - 移除按钮上移逻辑，改为星星自然挤压按钮位置
  - 无星星时容器自动隐藏不占空间
...
- **按钮间距调整**："复制题目"和"问豆包"按钮间距优化为5px，更紧凑
- **相对定位**：错题按钮改为相对定位，自然融入题目流
- **平滑过渡**：按钮上移动画0.3s缓动，视觉体验更佳
...
- **独立存储**：错题记录存储在mistakes store，不影响笔记数据

### 📝 笔记键结构优化
- **新键结构**：`CourseId_ClassId_WorkId_QuestionId_QuestionNo`
  - 旧格式：`217713435_125714113_45418488_question402709106`
  - 新格式：`217713435_125714113_45418488_question402709106_24`（24为题号）
- **题号智能解析**：自动从题目标题中解析题号（如"24. (单选题)"中的"24"）
- **降级处理**：解析失败时题号记为"999"
- **笔记管理优化**：笔记列表显示格式为"Question402709106_No24"，一目了然

### 🗄️ 数据库升级
- **版本升级至v4**：新增mistakes store用于错题记录
- **索引优化**：添加workKey、questionId、questionNo、count索引
- **向后兼容**：旧笔记数据不受影响，新笔记自动使用新键结构
...
### 📝 导出格式配置化 (v3.8.5.2)
- **格式选择**：在导出设置中选择DOC或DOCX格式，按需导出
- **DOC格式（默认）**：兼容性更好，适合手机/平板直接查看
- **DOCX格式**：标准Office文档，桌面端推荐使用
  - ⚠️ **注意**：DOCX在手机/平板显示可能出现bug，推荐使用DOC格式
- **统一配置**：2个导出按钮（试题/答案）共用格式配置

### 🎨 导出按钮简化
- **📄 导出试题**：根据配置的格式导出（不含答案）
- **📝 导出答案**：根据配置的格式导出（含答案）
- **一键切换**：在导出设置中选择格式，立即生效

### 🔧 技术优化
- **配置持久化**：导出格式保存到IndexedDB，下次自动使用
- **代码精简**：移除冗余的手机版按钮，统一为格式配置
- **用户体验**：清晰的格式说明和警告提示
...
- ⚙️ **配置持久化**：导出格式保存到IndexedDB，下次自动使用
- ⚠️ **警告提示**：DOCX格式标注"手机/平板可能有显示bug"
- 🔧 **代码优化**：移除冗余代码，提升可维护性
