...
提升超星学习通刷题效率的强大工具！支持**答案自由控制**（单个/全局）、**一键问豆包AI**（智能跨域提问+会话复用）、**富文本笔记**、**完整样式自定义**、**双格式试题导出**（DOCX/DOC-手机版），让你在作业练习中更专注、更高效。

## ✨ v3.8.5.1 导出功能优化

### 📱 新增手机版导出
- **4按钮导出系统**：DOCX试题、DOCX答案、DOC试题(手机版)、DOC答案(手机版)
- **DOC格式优势**：兼容性更好，适合手机直接查看，无需转换
- **统一配置**：4个导出按钮共用一套配置（字体、字号、行高、边距、导出内容选项）
- **纵向排列**：按钮从上到下依次排列，清晰明了

### 🎨 导出按钮说明
- **📄 导出试题**：DOCX格式，高质量标准文档（不含答案）
- **📝 导出答案**：DOCX格式，高质量标准文档（含答案）
- **📱 导出试题(手机版)**：DOC格式，兼容性强，适合移动端（不含答案）
- **📱 导出答案(手机版)**：DOC格式，兼容性强，适合移动端（含答案）

### 🔧 技术实现
- **DOCX导出**：使用html-docx-js库转换，生成标准Office文档
- **DOC导出**：直接使用HTML格式的.doc文件，跳过转换步骤，更快更稳定
- **共享配置**：两种格式使用相同的导出配置和内容选项
- **智能处理**：图片自动下载嵌入、超宽自动缩放、格式完整保留
...
### 📱 核心改进
- **移动端适配优化**：新增移动端设备检测，针对性优化豆包AI自动发送功能
- **差异化等待策略**：
  - 手机端：固定等待3秒让页面充分加载，提升稳定性
  - 桌面端：保持原有动态等待逻辑（MutationObserver），高效快速
- **智能设备识别**：通过User Agent自动识别移动设备（Android、iPhone、iPad等）
- **日志优化**：显示当前设备类型和工作模式，便于调试

### 🔧 技术细节
- **设备检测**：`/Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent)`
- **移动端策略**：`await new Promise(resolve => setTimeout(resolve, 3000))` + 直接获取元素
- **桌面端策略**：`waitForElement()` - MutationObserver动态监听DOM变化
- **错误处理优化**：移动端等待3秒后若仍未找到元素，抛出明确错误信息
...
### v3.8.4.3 兜底修复
- **完全采用硬编码方案**：移除所有临时变量依赖
- **移除 composed 参数**：避免移动端兼容性问题
- **恢复解锁发送按钮逻辑**：确保发送按钮在所有环境下可用

### v3.8.4.2 移动端兼容性增强
- **改用 Enter 键发送**：替代点击按钮，兼容桌面端和移动端
- **KeyboardEvent 模拟**：同时触发 keydown 和 keyup 事件
- **保留输入逻辑**：继续使用 `document.execCommand` 填充内容

### v3.8.4.1 权限修复
- **添加 GM_deleteValue 授权**：修复缺少 `@grant GM_deleteValue` 声明的问题
- **完善权限清单**：确保所有 GM API 都正确声明
...
### 🔧 核心改进
- **彻底移除不稳定的持续监听方案**：移除了豆包页面的 500ms 轮询逻辑，避免时序问题导致的"有时打开了却没有输入内容"
- **真正的会话复用**：通过配置会话ID实现稳定可靠的对话复用
- **配置读取优化**：从 IndexedDB 实时读取用户保存的前后缀配置，确保配置立即生效
- **简化豆包页面逻辑**：只在页面加载时执行一次自动填充，更稳定可靠

### 🤖 AI提问管理 - 会话复用（新增）
- **会话ID配置**：在"AI提问管理"中配置固定的豆包会话ID（可选）
- **使用方法**：
  1. 打开豆包AI创建新对话，复制URL中的数字ID
  2. 例如：`https://www.doubao.com/chat/32898162890824194` → 复制 `32898162890824194`
  3. 在控制面板 → AI提问管理 → 豆包会话ID 中粘贴
  4. 保存后，每次点击"问豆包"都会打开同一个会话
- **两种模式**：
  - 留空会话ID：每次新建标签页（默认行为）
  - 配置会话ID：每次打开同一会话，保持对话上下文
- **浏览器原生支持**：利用浏览器的标签页管理机制，自动聚焦已打开的标签页
...
### 📄 试题导出
- **双格式导出**：DOCX标准格式 + DOC手机版格式
- **4按钮系统**：DOCX试题/答案 + DOC试题/答案(手机版)
- **真正的 docx**：优先使用 html-docx-js 生成标准 docx
- **DOC兼容版**：直接生成HTML格式的.doc，兼容性更好，适合手机
- **多题型支持**：单选题、多选题、完型填空等
- **图片处理**：自动下载嵌入，超宽自动缩放
...
- 📝 **清晰标识**：手机版按钮使用📱图标，并标注"手机版"便于识别
- ⚡ **性能优化**：DOC格式跳过html-docx-js转换，生成更快

### v3.8.5 (2025-12-09) 📱 移动端优化
- 📱 **移动端设备检测**：新增User Agent检测，自动识别移动设备
- ⏱️ **差异化等待策略**：手机端固定等待3秒，桌面端保持动态等待
- 🎯 **提升稳定性**：移动端采用固定等待确保页面充分加载
- 📊 **日志优化**：显示设备类型和工作模式，便于调试
...
### v3.8.4.2 (2025-12-09)
- ⌨️ **改用 Enter 键发送**：替代点击按钮，兼容桌面端和移动端
- 🎹 **KeyboardEvent 模拟**：同时触发 keydown 和 keyup 事件
...
- 🗑️ **移除不稳定监听**：移除豆包页面的 500ms 轮询逻辑
- 🔄 **会话复用**：通过配置会话ID实现稳定可靠的对话复用
- ⚙️ **配置实时读取**：从 IndexedDB 实时读取用户保存的前后缀配置
- 🎯 **简化豆包逻辑**：只在页面加载时执行一次自动填充

### v3.8.1 (2025-12-08)
- 🤖 **AI提问管理选项卡**：新增"🤖 AI提问管理"选项卡
- 📝 **前后缀配置**：支持自定义AI提问前缀/后缀提示词
- 👁️ **实时预览**：配置面板中提供完整的效果预览
- 🗑️ **移除默认前缀**：默认传送完整题目，无额外内容
