...
提升超星学习通刷题效率的强大工具！支持**答案自由控制**（单个/全局）、**一键问豆包AI**（智能跨域提问+自动会话复用）、**富文本笔记**、**完整样式自定义**、**试题导出为Word文档**，让你在作业练习中更专注、更高效。

## ✨ v3.9.0 智能会话复用

### 🚀 核心特性
- **真正的自动复用**：无需手动配置会话ID，首次打开后自动保存网站分配的会话ID
- **智能判断**：优先使用配置的会话ID，未配置则新建并自动保存
- **立即生效**：保存后下次点击"问豆包"立即使用新会话
- **完全兼容**：保留原有配置逻辑，向后兼容v3.8.2版本

### 🔧 技术实现
- **URL监听**：豆包页面自动监听URL变化，提取会话ID
- **跨标签页通信**：使用 `GM_addValueChangeListener` 实现超星和豆包页面实时通信
- **持久化存储**：会话ID自动保存到 IndexedDB，永久有效
- **浏览器原生支持**：利用浏览器的标签页管理机制，自动聚焦已打开的标签页

### 📝 使用说明
1. **首次使用**：点击"问豆包"打开新会话，脚本自动保存分配的会话ID
2. **后续使用**：自动打开上次的会话，保持对话上下文
3. **手动更换会话**（可选）：
   - 在豆包AI创建新对话，复制URL中的数字ID
   - 例如：`https://www.doubao.com/chat/32898162890824194` → 复制 `32898162890824194`
   - 在控制面板 → AI提问管理 → 豆包会话ID 中粘贴并保存
4. **无需开关**：自动复用机制强制启用，无需额外配置
