...
>**<mark>项目永久开源，谨以此向22级软卓cg学长对期末资料的辛劳整理和伟大开源予以崇高致敬和由衷感谢**
...
### 🔧 v3.13.5 - 豆包发送按钮选择器修复 (2024-12-12)
- **🎯 按钮选择器分离**：区分发送按钮和状态检测选择器
  - 发送按钮：`button[data-testid="chat_input_send_button"]` - 用于点击发送
  - 状态检测：`#flow-end-msg-send` - 用于轮询检测aria-disabled状态
  - 修复点击错误按钮导致发送失败的问题

### 🔧 v3.13.3 - 豆包输入稳定性优化 (2024-12-12)
- **🖼️ 图片粘贴修复**：采用temp7的正确实现方式
  - 使用`document.createEvent` + `initEvent`创建paste事件
  - 添加200ms等待确保粘贴完成
  - 修复图片无法成功输入的问题
- **⏱️ 简化加载等待**：去除MutationObserver，使用固定等待
  - 固定1.5秒页面加载等待，更稳定可靠
  - 避免Observer带来的不确定性
- **🔗 URL匹配优化**：修复指定会话ID时无法触发的问题
  - 将`@match`从`/chat/*`改为`/chat*`
  - 支持`/chat`和`/chat/123456`等所有chat路径

### ⚡ v3.13.2 - 豆包页面加载优化 (2024-12-12)
- **🔍 MutationObserver自动等待**：智能监听DOM变化，无固定延迟
  - 元素出现立即执行，无需固定等待1.5秒
  - 10秒超时保护，避免无限等待
  - 提升加载速度和响应效率
- **🔓 发送按钮解锁机制**：确保按钮可点击
  - 移除disabled属性
  - 设置aria-disabled为false
  - 恢复pointerEvents为auto
  - 避免按钮被禁用导致发送失败

### 🔧 v3.13.1 - 豆包输入逻辑优化 (2024-12-12)
- **🎯 场景智能识别**：自动判断纯文字/纯图片/混合场景
  - 纯文字：仅发送文字内容
  - 纯图片：仅发送图片内容（支持多图）
  - 混合内容：先发送文字，再依次粘贴图片
- **🔓 输入框解锁机制**：采用4步解锁流程防止内容清空
  - 模拟人工聚焦（click + focus）
  - 输入空格触发初始状态
  - 选中并删除空格（select + backspace）
  - 使用document.execCommand输入实际内容
- **✨ 文字输入修复**：解决纯文字场景输入被清空的问题
  - 使用`document.execCommand('insertText')`替代直接赋值
  - 确保所有场景（纯文字/纯图片/混合）都正确执行解锁流程

### 🎉 v3.13.0 - 智能AI多模态升级 (2024-12-12)
- **🖼️ 多图支持**：问豆包AI现支持自动提取题干中的所有图片
  - 自动识别题目中的`<img>`标签并下载图片
  - 支持多张图片同时发送（题干可能包含多图）
  - 智能跳过下载失败的图片，确保流程不中断
- **📦 分片存储优化**：实现大图片的分片存储机制
  - 2MB分片大小，突破单个GM_setValue存储限制
  - 元信息+分片数据分离存储，支持多图管理
  - 自动清理机制，避免存储空间浪费
- **⚡ 轮询检测算法**：豆包页面发送按钮智能检测
  - 0.1秒高频轮询，精准捕捉按钮可用状态
  - 10秒超时保护，失败时弹窗提示用户手动发送
  - 检测`aria-disabled`属性，确保发送时机准确
- **🔄 混合内容自动填充**：文字+多图一键发送
  - 文字优先输入，图片依次粘贴（无间隔）
  - 所有内容输入完成后启动轮询检测
  - 自动点击发送或超时提示，提升用户体验
...
### 🤖 智能AI助手（多模态支持）
- **一键问豆包**：点击"🤖 问豆包"按钮，自动提取题目内容（含文字+图片）
- **多图支持**：自动识别并下载题干中的所有图片，支持多图同时发送
- **跨域提问**：自动打开豆包AI并填充文字+图片到输入框
- **智能发送**：0.1秒轮询检测发送按钮状态，自动点击发送或10秒超时提示
- **会话复用**：智能检测已有对话窗口，优先复用
- **前后缀配置**：支持自定义题目前后缀提示词（如："请帮我解答："）
...
- **简化核心逻辑**：从复杂的DOM操作简化为直接样式属性操作
- **清理冗余代码**：移除`originalHTML`、`currentAnswerBlock`等旧变量
- **提升性能**：减少DOM操作，提高响应速度和稳定性

### v3.9.1.5 (2025-12-10) 🐛 豆包自动发送修复
- **简化等待逻辑**：移除设备检测和动态等待，统一使用1.5秒固定等待
- **提升稳定性**：确保页面加载完成后再操作元素

### v3.9.1.4 (2025-12-10) 🐛 豆包自动发送优化
- **固定等待时间**：采用1.5秒固定等待，移除复杂的设备检测逻辑
